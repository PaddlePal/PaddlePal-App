...
// into 5 zones (from the hardware diagram; the old Zone 6 is merged into Zone 5).
// Zones are positioned by GRID FRACTIONS of the head's bounding box, so they
// scale with the drawn paddle. Columns 1=left … 4=right, rows 1=top … 5=bottom.
//
// ⚠️ Which physical region maps to firmware zone IDs 1–5 is still unconfirmed on
...
//   Z5 top    (row 1   × cols 2–3, 2 sensors)
const COLS = 4;
const ROWS = 5;
...
const ZONES: ZoneConfig[] = [
  { zone: 5, col: [2, 3], row: [1, 1] },
  { zone: 3, col: [1, 1], row: [2, 4] },
  { zone: 1, col: [2, 3], row: [2, 3] },
  { zone: 4, col: [4, 4], row: [2, 4] },
  { zone: 2, col: [2, 3], row: [4, 5] },
];

const ZONE_ORDER: ZoneId[] = [1, 2, 3, 4, 5];

/** Converts a zone's grid span into a percentage-positioned box within the head. */
function zoneBox({ col, row }: ZoneConfig): ViewStyle {
